...
export function Dashboard() {
  ...
  const kpis=[{v:nf(total),l:t("total_events"),icon:<ServerIcon/>,c:"#D97706",bg:"var(--td-brand-color-light)"},{v:total>0?`${((ok/total)*100).toFixed(1)}%`:"—",l:t("system_health"),icon:<SecuredIcon/>,c:ok>=total*.95?"#34D399":"#FBBF24",bg:ok>=total*.95?"var(--td-success-color-1)":"var(--td-warning-color-1)"},{v:`${(data?.elapsedMs??0).toFixed(1)}ms`,l:t("query_latency"),icon:<ThunderIcon/>,c:"#60A5FA",bg:"var(--td-brand-color-light)"},{v:eps>0?`${nf(eps)}/s`:"—",l:t("write_throughput"),icon:<ThunderIcon/>,c:"#D97706",bg:"var(--td-brand-color-light)"}];
        ...
        <Row gutter={[12,12]}>
          <Col span={12}>
            <Card bordered title={t("write_throughput")} style={{height:"100%"}}>
              ...
              <ResponsiveContainer width="100%" height={200}><AreaChart data={chart} margin={{top:4,right:4,left:-12,bottom:0}}><defs><linearGradient id="ga" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#D97706" stopOpacity={.15}/><stop offset="100%" stopColor="#D97706" stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke="var(--td-component-stroke)" vertical={false}/><XAxis dataKey="t" tick={{fontSize:9}} stroke="var(--td-text-color-placeholder)" interval={5} tickLine={false}/><YAxis tick={{fontSize:9}} stroke="var(--td-text-color-placeholder)" width={32} axisLine={false} tickLine={false}/><Tooltip contentStyle={{borderRadius:8,fontSize:11}} formatter={(v:number)=>[`${v} events`]}/><Area type="monotone" dataKey="v" stroke="#D97706" strokeWidth={1.5} fill="url(#ga)" animationDuration={400}/></AreaChart></ResponsiveContainer>}
            </Card>
          </Col>
          <Col span={12}>
